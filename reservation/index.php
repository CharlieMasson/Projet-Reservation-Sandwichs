...
PHP
 ...
 if(isset($_POST["submit-btn"]))
 {
	...
	$db = connexion();
	$requete = $db->prepare("SELECT active_user FROM utilisateur WHERE id_user = ?");
	$requete -> execute(array($_SESSION['id_utilisateur']));
	$utilisateur = $requete->fetch();
	if ($utilisateur['active_user'] == 0){
		$erreur = "Erreur: Votre compte utilisateur n'est plus actif";
		$isSuccess = false;
	}
...
<body>
	<div id="formulaire" class="section">
		...
		<a href="../index.php"><button class="btn" id="btn-retour">Retour à l'accueil</button></a>		
		</div>
	</div>
